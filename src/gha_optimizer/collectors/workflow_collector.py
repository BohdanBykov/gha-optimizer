...
class WorkflowCollector:
    ...
    def collect_workflow_data(self, owner: str, repo: str, token: str, max_history_days: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if not github_client.test_connection():
                raise GitHubAPIError("GitHub API connection test failed - check your token and network connectivity")
            ...
            self.logger.info(f"Successfully collected data: {len(workflows)} workflows, {len(workflow_runs)} runs")
    ...
    def get_raw_workflows_for_ai(self, owner: str, repo: str, token: str) -> Dict[str, str]:
        ...
        try:
            ...
            return {
                workflow.file_path: workflow.content 
