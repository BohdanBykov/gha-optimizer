...
class ScanCommand:
    ...
    def execute(
        ...
        try:
            ...
            workflow_data = workflow_collector.collect_workflow_data(owner, repo, token, max_history_days)
            
            # Get raw workflows for AI analysis
            raw_workflows = workflow_collector.get_raw_workflows_for_ai(owner, repo, token)
            
            # Run AI analysis on raw workflows
            ...
            estimated_savings, time_savings = self._calculate_impact_from_ai_recommendations(ai_recommendations)
            
            recommendations = ai_recommendations
            
            self.logger.info(f"Analysis completed. Found {len(recommendations)} optimization opportunities")
            ...
            report = reporter.generate_report(repository, recommendations, estimated_savings, time_savings, workflow_data)
            print(report)
    ...
    def _calculate_impact_from_ai_recommendations(self, recommendations: List[Dict[str, Any]]) -> tuple[float, float]:
